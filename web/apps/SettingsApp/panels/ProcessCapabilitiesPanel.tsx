...
import { GroupCollapsible, Menu, type MenuItem, Label, Button, Card, CardItem, Text } from '@cypher-asi/zui';
import {
  ArrowLeftRight,
  Keyboard,
  HardDrive,
...
import type { CapabilityInfo, ObjectType } from '../../../components/AppPermissions';
...
export function ProcessCapabilitiesPanel({ 
  ...
  const handleRevoke = useCallback((objectType: ObjectType) => {
    ...
    if (success) {
      console.log(`[ProcessCapabilitiesPanel] Revoked ${objectType} from PID ${app.pid} slot ${cap.slot}`);
    } else {
      console.error(`[ProcessCapabilitiesPanel] Failed to revoke ${objectType} from PID ${app.pid}`);
    }
  ...
  const handleRevokeAll = useCallback(() => {
    ...
    console.log(`[ProcessCapabilitiesPanel] Revoked ${successCount}/${app.grantedCaps.length} caps from PID ${app.pid}`);
