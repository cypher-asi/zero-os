...
export function PermissionsPanel({ onDrillDown }: PermissionsPanelProps) {
    ...
    const fetchProcesses = () => {
      try {
        ...
        if (typeof supervisor.get_processes_with_capabilities_json !== 'function') {
          console.error('[PermissionsPanel] get_processes_with_capabilities_json is not a function - WASM may need rebuild');
  ...
  const handleSelectApp = useCallback((app: AppWithPermissions) => {
    if (onDrillDown) {
      onDrillDown({
        id: String(app.pid),
        label: app.manifest.name,
        content: (
          <ProcessCapabilitiesPanel 
            app={app} 
  ...
  }, [onDrillDown, supervisor, triggerRefresh]);
