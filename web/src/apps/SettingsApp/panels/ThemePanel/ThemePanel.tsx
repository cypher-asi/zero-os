...
import { useBackground } from '@desktop/Desktop';
...
export function ThemePanel() {
  ...
  const accentItems: MenuItem[] = ACCENT_COLORS.map((color) => ({
    ...
    icon: <span className={styles.colorDot} style={{ backgroundColor: ACCENT_HEX[color] }} />,
  }));
      ...
      >
        <div className={styles.menuContent}>
          <Menu
            items={accentItems}
            value={accent}
            onChange={(id) => setAccent(id as AccentColor)}
            background="none"
            border="none"
          />
        </div>
