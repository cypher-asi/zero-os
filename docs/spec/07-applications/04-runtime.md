...
- Provides the event loop that drives `OrbitalApp` implementations
...
```rust
/// Runtime environment for an Orbital application.
...
impl AppRuntime {
    ...
    pub fn run<A: OrbitalApp>(&mut self, mut app: A) -> ! {
...
```rust
/// Generate the entry point and runtime setup for an Orbital app.
#[macro_export]
...
```rust
use orbital_apps::{app_main, OrbitalApp, AppContext, ControlFlow, AppError, Message};
...
unsafe fn raw_syscall(num: u32, a0: u32, a1: u32, a2: u32, a3: u32) -> u32 {
    ...
    extern "C" {
        fn orbital_syscall(num: u32, a0: u32, a1: u32, a2: u32, a3: u32) -> u32;
    }
    orbital_syscall(num, a0, a1, a2, a3)
}
```

The `orbital_syscall` function is injected by the WASM runtime and routes to the kernel via `postMessage`.
