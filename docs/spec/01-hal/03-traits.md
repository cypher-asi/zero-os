...
```rust
// orbital-hal-wasm/src/lib.rs (simplified)
...
```rust
// orbital-hal-native/src/lib.rs (future Phase 2)
...
The existing `orbital-hal` crate already defines this trait with the same interface. The spec documents the expected behavior that implementations must provide.
