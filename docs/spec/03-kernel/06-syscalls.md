...
> The canonical system call interface for Orbital OS.
...
To maintain compatibility with existing `orbital-process` code, the kernel accepts legacy syscall numbers and maps them to the canonical ABI:
...
```rust
//! Syscall numbers for Orbital OS.
